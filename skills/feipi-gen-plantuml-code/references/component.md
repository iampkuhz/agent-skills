...
**禁止所有元素使用相同矩形框**。必须使用以下语法区分：

| 元素类型 | 形状 | 颜色 | PlantUML 语法 |
|----------|------|------|---------------|
| 组件 | 矩形 | 蓝色系 | `component [名称]` |
| 外部角色 | 人形 | 灰色系 | `actor [名称]` |
| 数据存储 | 圆柱体 | 绿色系 | `database [名称]` |
| 数据对象 | 矩形 (note) | 黄色系 | `note "名称" as 别名` |

**样式库引用**：
```plantuml
!include architecture-diagram-styles.puml
```
