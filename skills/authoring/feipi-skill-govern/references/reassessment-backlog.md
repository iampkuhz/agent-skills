...
说明：
- 本清单只记录待重审项，不在当前自检中顺手修改其他 skills。
...
- 处理：已在 2026-04-06 重审并落地为 `skills/integration/feipi-dingtalk-send-webhook/`，本条不再属于待执行项。
...
- 所有基于旧 action-first 命名推导出的 Step 2C / Step 2D 方案。
- 所有把 `web`、`ops`、`automate` 当核心 action 的未实施建议。
