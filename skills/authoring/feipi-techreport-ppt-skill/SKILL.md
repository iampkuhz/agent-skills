...
**Production 模式**：必须生成决策确认卡片，等待用户确认。

**Draft 模式**：可生成简化版 `Draft Contract`，仅包含主题、结论、内容范围，减少确认环节，直接出可讨论的版本。但输出必须标记为草稿，不得伪装成最终版。

#### 内部规划（不展示给用户）

LLM 内部仍需完成完整 Page Contract 规划（目标、结论、原始信息映射、页面结构、版面蓝图、容量预算），作为后续 Slide IR 生成的依据。但**不要将完整 Page Contract 原样展示给用户**。

#### 用户可见输出：决策确认卡片

用户可见输出改为**决策确认卡片**，控制在约 15 行以内。格式如下：
...
- 只在**真正不确定、会影响产出**的地方提问，例如：
  - 对象是否压缩或拆页；
  - 主画面优先级；
  - 哪些信息进入主表、脚注或第二页；
  - 结论聚焦方向存在多个合理选项时的取舍。
- 每个决策点必须提供 **2-3 个候选选项**，标注一个推荐项。
- 不要让用户开放式回答"你想怎么做"，而是让用户选择候选或接受默认组合。
- 不要重复列出用户已提供的全部原始信息，只做一句极简归纳。
- 不要展示内部版面蓝图的完整百分比分配，除非该分配本身需要用户确认。
- **当没有明显不确定性时**，只输出简短确认：
  - "信息足够，我将按「[版式名称]」生成，重点突出「[结论]」。确认后开始生成。"

**生成前确认不可跳过**：Production 模式下，必须等用户确认决策卡片后，才可进入 Slide IR 生成和 PPTX 编译。

详细 Page Contract 内部格式和容量预算规则见 `references/page-contract.md`。
