...
- 非 `feipi-gen-skills` 的 `SKILL.md` 禁止出现仓库维护命令（如 `make validate`、`make test`）。
- 模板产物不得残留 `{{...}}` 之类未替换占位符。

## 校验约束
- 新建或修改 skill 后，必须执行：`make validate DIR=<skill-root>/<name>` 或等价 wrapper。
...
- 只要调整了目录标准、初始化逻辑、测试入口或校验规则，就要同步检查：
  - `templates/`
  - `feipi-scripts/repo/*.sh`
  - `skills/feipi-gen-skills/scripts/*.sh`
