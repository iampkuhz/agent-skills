...
if [[ "$HAS_TIMESTAMPS" -eq 1 ]]; then
  timestamp_rules="如引用视频证据，时间锚点格式只能是：
   - 时长未超过 1 小时： [MM:SS]
   - 时长超过 1 小时： [HH:MM:SS]
   禁止使用 T+00:00:00，禁止使用字幕行号。"
else
  timestamp_rules="当前转写文本不含可靠时间戳（HAS_TIMESTAMPS=0）。如引用视频内容，禁止编造 [MM:SS] 或 [HH:MM:SS] 格式的时间标记。可以使用'视频提到'或直接引用，不使用伪造时间前缀。"
fi

# --- 证据披露规则 ---
evidence_disclosure="禁止空泛免责；必须披露真实证据状态：
...
# --- 来源状态要求 ---
source_status_rule="必须在输出中包含 '## 来源状态' 章节（若 expand 模式，附加在 '## 相关影响和背景分析' 之后；若 background-only 模式，附加在 '## 上下文背景' 之后），且包含以下字段：
...
12) ${evidence_disclosure}
13) ${source_status_rule}
...
8) ${evidence_disclosure}
9) ${source_status_rule}
