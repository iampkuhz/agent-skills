...
示例:
  ...
  bash scripts/validate.sh skills/integration/feipi-dingtalk-send-webhook
USAGE
}
...
done < <(rg -o 'references/[A-Za-z0-9._/-]+\.txt|scripts/[A-Za-z0-9._/-]+\.(sh|py)' "$SKILL_FILE" | sort -u)
