...
EXTRA_MESSAGE_DIAGRAM="$SCRIPT_DIR/tests/extra-message-diagram.puml"
...
if python3 "$COVERAGE_SCRIPT" --brief "$VALID_BRIEF" --diagram "$EXTRA_MESSAGE_DIAGRAM" >/dev/null 2>&1; then
  echo "额外消息未被覆盖校验拦截" >&2
  exit 1
fi

if python3 "$COVERAGE_SCRIPT" --brief "$VALID_BRIEF" --diagram "$MISSING_SEPARATOR_DIAGRAM" >/dev/null 2>&1; then
  echo "缺失 separator 未被覆盖校验拦截" >&2
  exit 1
fi
