...
示例:
  ...
  bash scripts/validate.sh skills/diagram/feipi-plantuml-generate-architecture-diagram
USAGE
}
...
done < <(rg -o 'references/[A-Za-z0-9._/-]+\.md|assets/[A-Za-z0-9._/-]+(\.md|\.yaml|\.json|\.puml|\.txt)|scripts/[A-Za-z0-9._/-]+\.(sh|py)' "$SKILL_FILE" | sort -u)
