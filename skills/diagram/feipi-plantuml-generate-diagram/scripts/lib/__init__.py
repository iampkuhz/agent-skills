#!/usr/bin/env python3
"""统一 validation.json 写入工具，避免每个脚本拼一份不同结构。"""
...
class ValidationResult:
    ...
    def set_success(self) -> None:
        ...
        self.blocked_reason = ""
        if self.render_result == "ok":
            if self.brief_check == "skipped":
                self.brief_check = "ok"
            if self.coverage_check == "skipped":
                self.coverage_check = "ok"
            if self.layout_check == "skipped":
                self.layout_check = "ok"
