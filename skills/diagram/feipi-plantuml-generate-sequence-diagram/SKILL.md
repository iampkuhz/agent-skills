...
3. Implement：
   ...
   - 图中的每一条箭头消息都必须来自 brief，不得额外补返回消息或通知消息；`activate/deactivate` 仅作为布局辅助，可按调用栈酌情添加。
   - 生成后必须执行 `bash scripts/validate_package.sh --brief <brief.yaml> --diagram <diagram.puml> --out-dir <output-dir>` 产出 diagram package。
   - **只有读取 `validation.json` 确认 `final_status=success` 且 `render_result=ok` 后，才能将 diagram package 交付给上游流程。**
...
- 若 `final_status=blocked`，必须根据 `blocked_reason` 修复后重新执行 `validate_package.sh`。
...
3. 图中的每一条箭头消息都必须来自 brief；`groups[].separator=true` 时必须落对应 `separator`。
