...
#
# 用法：
#   bash scripts/video/install_video_deps.sh            # 自动安装缺失依赖（支持 macOS + Homebrew）
#   bash scripts/video/install_video_deps.sh --check    # 仅检查，不安装
...
usage() {
  cat <<'USAGE'
用法:
  bash scripts/video/install_video_deps.sh [--check]
